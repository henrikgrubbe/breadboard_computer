...
import * as grammar from "@/assets/grammar.peggy?raw";
import type { GrammarInput } from "peggy";
...
export default defineComponent({
  ...
  data() {
    return {
      parser: peggy.generate(grammar.default as GrammarInput),
      source: "" as string,
      compiled: "" as string,
      error: " " as string,
    ...
    compiledHighlighter(code: string) {
      return code
        ...
        .map((line: string) =>
          line.replace(
            /(\d{4}):(\d{4})(\d{4})/,
            "<addr>$1</addr>:<instr>$2</instr><opcode>$3</opcode>",
          ),
        )
        .join("\n");
    },
